      ...
      <li v-for="(item, index) in lists" :key="index">
        ...
        <div class="orderAll-btn">
          ...
          <div v-if="item.type == 1">
            <van-button size="mini" type="primary" @click="onPay(item.orderId)"
              >支付订单</van-button
            >
          ...
          <div v-else-if="item.type == 2">
            ...
            <van-button size="mini" type="info" @click="onGoods(item.orderId)"
              >已收货</van-button
            >
          </div>
          <!-- 已完成 -->
          <div v-else-if="item.type == 3">
            <van-button size="mini" @click="evaluate = true"
              >商品评价</van-button
            >
          </div>
        </div>
...
export default {
  ...
  created() {
    ...
    this.$store.state.orderactive = 0;
  },
  methods: {
    onPay(orderId) {
